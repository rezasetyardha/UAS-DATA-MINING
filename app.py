...
try:
    ...
    decision_tree_model = pickle.load(open('DAR.pkl', 'rb'))
...
st.subheader('Hasil Prediksi Harga Smartphone (Decision Tree):')
...
if st.checkbox("Detail Kumpulan Dataset Fitur Smartphone"):
    st.subheader('Detail Dataset Fitur:')
    st.write(data)
    
    fig = px.pie(data, names='price_range', title='Distribusi Kisaran Harga Smartphone')
    st.plotly_chart(fig)
